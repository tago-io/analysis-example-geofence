...
async function startAnalysis(context, scope) {
  ...
  if (!zone) {
    ...
    notification.send({ title: "No zone alert", message: "Your device is not inside any zone." });
  ...
  if (zone.event === "danger") {
    ...
    notification.send({ title: "Danger alert", message: "Your device is inside a dangerous zone." });
  }
  if (zone.event === "safe") {
    // If our device is inside a safe geofence, we will send a safe geofence notification.
    notification.send({ title: "Sage alert", message: "Your device is inside a safe zone." });
  }
